...
public class databaseCreator {	
	...
	{		
		...
		ArrayList<String> repos = gitHubAPI.getRepositories();
		for (int i = 0; i < repos.size(); i++ ) {
			
			System.out.println("Creating Repository Node: " + repos.get(i) );
			neo4jAPI.createRepoNode ( neo4jAPI.getGitHubNode(), repos.get(i) );
		...
		// -------------------------------------- LEVEL 3 --------------------------------------
			...
			String repoId = "111";
			gitHubAPI.getRepoContributors( repoId, repoOwner, repos.get(i) );	// THROWS
			ArrayList<String> repoContributors = gitHubAPI.getArrayListOfContributors();
			ArrayList<Integer> repoContributorsCommits = gitHubAPI.getArrayListOfContributorsCommits();
